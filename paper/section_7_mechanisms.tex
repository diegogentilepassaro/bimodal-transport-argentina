...
%
% Numbers inline, verified against results/tables/table_14_mechanisms.csv
% and results/tables/diagnostic_heterogeneity.csv; AutoFill macros in the
% paper-wide substitution pass.
%
...
first paved road (82 districts). The kilometer measures are computed
...
reproduces the baseline estimate of 0.046 (0.033). Adding the two
kilometer measures in column~(2) lowers the market-access coefficient
to 0.028 (0.031); adding the two binary margins alone in column~(3)
leaves it at 0.052 (0.032); and including all four $Z_i$ jointly in
column~(4) lowers it to 0.021 (0.031). The first-stage $F$ rises
mildly across columns, from 13.6 to 17.5, so the attenuation is not a
weak-instrument artifact.
...
positively and precisely (0.0015, standard error 0.0004, in
...
kilometers. The two readings are not competing hypotheses; they are
the same estimate described from its two ends, and the data do not
distinguish a preferred one.
...
enters positively (0.209, standard error 0.113) in column~(4). With
...
rural population share in 1960, and distance to Buenos Aires. Each
specification adds the interaction of the characteristic with
$\Delta \ln \mathrm{MA}^{\mathrm{full}}_i$ to the main equation,
instrumenting the interaction with the interactions of both
instruments with the characteristic.

The ordinary-least-squares interaction coefficients align with this
expectation in all three cases: negative for initial population
($-0.022$, $p = 0.09$), positive for the rural share ($0.028$,
$p = 0.02$), and positive but imprecise for distance to Buenos Aires
($0.013$, $p = 0.37$). Under instrumental variables the signs are
