...
districts with missing kilometer measures (\mechNAug{} versus \mechNBase{}
observations); re-estimated on the common \mechNAug{}-district sample, the
...
Three caveats apply. First, because the $Z_i$ are not instrumented,
...
reform. Second, when several treatment-like regressors enter one
regression, each coefficient can absorb a non-convex average of the
others' heterogeneous effects
\citep{goldsmithpinkhamhullkolesar2024}; this reinforces reading the
column as description rather than as a set of separately identified
effects. Third, the indicator for losing the entire rail network
...
market-access level weakens to an $F$ between \heteroFMaMin{} and
\heteroFMaMax{} once the interaction enters, because the instrument
set must now separate two endogenous regressors. We therefore read these results as sign patterns
