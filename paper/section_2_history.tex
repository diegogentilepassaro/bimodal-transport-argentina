...
road network expanded through new construction, while the rail
network contracted through the closure of existing lines. The two
margins need not have symmetric local effects. A closure removes a
service around which firms, storage facilities, and settlements had
already located, so its effects operate through the depreciation of
place-specific investments; new construction adds access where the
complementary investments do not yet exist, so its effects operate
through entry. If adjustment to losses is slower than adjustment to
gains --- structures and settled populations are durable --- the
same absolute change in access can produce responses that differ
with its sign.
...
interior to the port of Buenos Aires (Section~\ref{ssec:railway_history});
many interior region pairs were connected only through that hub.
Roads branch more densely and serve shorter, more varied routes, so
the road expansion connected interior districts to one another as
well as to the port. A contraction of the radial network combined
with an expansion of the capillary one therefore changes not only
the level of a district's market access but also its composition
--- which destinations become cheaper to reach --- in ways a single
aggregate index summarizes only partially.

Whether these differences --- in cost structure, in the margin of
change, and in network geometry --- generate meaningful
...
motivate the density channel directly; the closure-versus-construction
and geometry channels are stated as context for interpreting the
