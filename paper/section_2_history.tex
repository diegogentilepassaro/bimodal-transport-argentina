...
equipment.\footnote{This figure is from a national track-kilometre
series and is not comparable to the network totals reported later in
this section, which come from our digitised map
(Section~\ref{ssec:networks}). The map runs about 2.5~percent below that
series uniformly across the periods we can compare, a difference in
measurement rather than in coverage. We report the map throughout
because it is the source the estimation uses; we quote the series here
only because the map, a 1979 snapshot, carries no information about the
network before 1960.}
...
by the Ministry of Public Works and Services in 1962 as
\emph{Transportes Argentinos: Plan de Largo Alcance} (``Argentine
Transportation: Long-Run Plan''), comprised three volumes totaling
approximately 3{,}000 pages.\footnote{The report's cover and its
letter of elevation to the minister are dated 1962; President
Frondizi announced the plan's recommendations in 1961, before the
full report was issued.} The first volume contained an executive summary with
...
year of cargo entering the network through a given segment. By the
report's own accounting, only 39.6~percent of the railroad network
was studied. For each studied
...
nonstudied ones—a classification that we exploit for identification.
...
Our rail source separates closures into two groups rather than dating
them.\footnote{The source is a single 1979 network map whose segments
carry a closure status rather than a closure date
(Section~\ref{ssec:networks}): each segment is recorded as closed before
1976, closed under the dictatorship, or still active. It therefore
supports the two groups reported here but cannot say when, within either
window, a given closure happened, and we do not apportion these
kilometers across years.} \railKmClosedPreSeventySix{} kilometers of
track were closed before 1976, beginning with the roughly
1{,}000 kilometers that followed the Larkin Plan's announcement. A
further \railKmDictatorship{} kilometers were closed under the military
dictatorship that seized power in 1976, which without union resistance
was able to act where earlier governments had been stopped. No new rail
construction followed, and the map records each surviving segment as
active in both 1979 and 1986, so the network's 1986 extent is observed
rather than inferred. In
total, the railroad network fell from
\railKmSixty{} kilometers in 1960 to \railKmEightySix{} kilometers by
1986, a reduction of \railPctFall{}~percent.\footnote{We end our sample
...
to the remaining railroad network.\footnote{Figures in this
subsection come from the Dirección Nacional de Vialidad series for
the national road network \citep{dnvseries}: 27{,}276~km paved plus
7{,}153~km gravel in 1986, excluding dirt roads. The digitized road
maps used in the empirical analysis (Section~\ref{sec:data}) cover
the broader network, including provincial roads, and therefore
report larger totals.} Importantly, the spatial pattern
...
The cost structure is not the only conceptual difference between
the two shocks. A second difference is the margin of change: the
road network grew through construction and the paving of existing
routes, while the rail network contracted through the closure of
existing lines. The two margins need not have symmetric local
effects. A closure removes a service around which firms, storage
facilities, and settlements had already located, so its effects
would operate through the depreciation of place-specific
investments; a new or newly paved road adds access whose
complementary investments are still partly to come, so its effects
would operate through entry and upgrading. If adjustment to losses
is slower than adjustment to gains—structures and settled
populations are durable—the same absolute change in access can
produce responses that differ with its sign.

A third difference is network geometry. The railroad network was
laid out radially, oriented to move export commodities from the
interior to the port of Buenos Aires
(Section~\ref{ssec:railway_history}), so many interior region pairs
were linked mainly through that hub; the road network is visibly
more capillary, connecting interior districts to one another as
well as to the port (Figure~\ref{fig:networks}). A contraction of
the radial network combined with an expansion of the capillary one
therefore changes not only the level of a district's market access
but also its composition—which destinations become cheaper to
reach—in ways a single aggregate index summarizes only partially.

Whether these differences—in cost structure (the density channel
developed above), in the margin of change, and in network
geometry—generate meaningful
mode-specific effects on regional economic outcomes is ultimately
an empirical question. It is not obvious a priori that they do: if
road expansion sufficiently compensated for rail closures, or if
firms and workers adjusted quickly to the new transport
configuration, the sectoral composition of economic activity might
not have changed. We treat the existence of mode-specific effects
as a hypothesis to be tested in
Sections~\ref{sec:results} and~\ref{sec:counterfactuals},
rather than as an established fact. The cost data presented here
motivate the density channel directly; the margin-of-change and
geometry channels are stated as context for interpreting the
mode-specific estimates, and our district-level data do not separate
the three channels from one another.
