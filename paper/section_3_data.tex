...
discounted in the MA sum. It is a structural input from the trade
literature, not a parameter estimated in this paper. Structural
...
values near 4, with a credible range of roughly 4 to 8; and
\citet{donaldson2018} estimates 3.8 from railroad-borne trade in
colonial India, the closest setting to ours in kind. We report
two values spanning this range, $\theta_{\mathrm{low}} = \thetaLow{}$
--- inside the modern range of \citet{simonovskawaugh2014} and
close to the \citet{donaldson2018} estimate --- and
$\theta_{\mathrm{high}} = \thetaHigh{}$, close to the
\citet{eatonkortum2002} preferred value. Main results use
$\theta_{\mathrm{low}}$; $\theta_{\mathrm{high}}$
appears in the sensitivity tables, and
Section~\ref{sec:discussion} reports how the population elasticity
varies over the full sweep $\theta \in [1, 12]$.
\placeholder{$\theta$ paragraph above is proposed text pending
coauthor sign-off; exact 4.55/8.11 provenance unconfirmed}
% PROPOSED TEXT (theta justification) — needs Cote's sign-off. Two open
% points flagged in the PR: (1) the exact decimals 4.55/8.11 have no
% pinned source in the project record (the planned draft even had their
% roles swapped: 8.11 main, 4.55 robustness); if they trace to a
% specific paper or an old calibration, cite it, otherwise consider
% rounding to literature-anchored values (e.g. 4 and 8.28). (2) The
% deeper theta-tau compatibility question (memo Decision A: D-H-sized
% exponent on raw generalized costs) is NOT resolved by this paragraph.
