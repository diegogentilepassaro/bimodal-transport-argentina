...
at least 15{,}000 in 1960.\footnote{Capital Federal is absent from the
node list although it satisfies both criteria, and although it does
enter every district's market access as a destination. The omission
comes from the city layer we digitised rather than from a modelling
choice, and it is immaterial: rebuilding the spanning tree with Capital
Federal added as a fifty-fifth node leaves the instrument unchanged to
six decimal places, with a maximum absolute deviation across districts
of zero at the precision we report. Five other Greater Buenos~Aires
municipalities are nodes, so the network is not one that routes around
the capital region.}
The first network links every pair of cities with a straight
line (1{,}431 segments); the second is the
