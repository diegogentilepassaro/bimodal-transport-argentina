...
Two mechanisms work in opposite directions. First, the state may have
closed rail lines serving districts that were already in decline, so
that lost market access coincides with falling outcomes for reasons
unrelated to the closure itself; this biases the ordinary-least-squares
estimate of $\beta$ downward. Second, paved roads may have been built
to serve districts that were expected to grow, so that gained market
access coincides with rising outcomes; this biases $\beta$ upward.
Because the two mechanisms push in opposite directions, the sign of the
net bias in ordinary least squares is ambiguous. We address the
...
The first instrument exploits the 1962 Larkin Plan, a government
technical study of the rail network. The Plan evaluated segments
against an engineering and freight-traffic standard and designated a
subset for study toward closure; we label these segments
\emph{studied} and the remainder \emph{non-studied}
...
The Larkin classification predicts realized closure, which gives the
instrument its first-stage power: of the rail segments the Plan
studied, 45 percent were closed by 1986, against 11 percent of
non-studied segments --- studied segments were roughly four times as
likely to close. Identification requires that the classification be
uncorrelated with district outcome shocks over 1960--1986, conditional
on the controls in equation~(\ref{eq:main}). The assumption is
plausible because the Plan applied a national, traffic-based
engineering rule fixed at the start of the period rather than a
forecast of regional growth. It is not guaranteed: if the traffic rule
selected segments in districts already on a distinct trajectory,
studied status would correlate with pre-existing trends.
Sections~\ref{sec:pre_balance} and~\ref{sec:pre_trends} probe this
directly.
...
build a minimum spanning tree --- the connected set of edges with the
lowest total cost --- which yields a parsimonious network that does
not depend on which individual city pairs happen to be linked. This
produces four variants: the full bilateral least-cost network (LCP)
and its spanning tree (LCP-MST), and the full Euclidean network (EUC)
and its spanning tree (EUC-MST). The main specification uses LCP-MST.
We construct $\Delta \ln \mathrm{MA}^{\mathrm{hypo}}_i$ as the change
in log market access induced by adding this hypothetical network.
...
over 1960--1986. It is relevant because actual road construction tended
to follow the same low-cost corridors connecting the same cities, which
gives the instrument first-stage power; it is plausibly excludable
because the least-cost routing reflects engineering geography and does
not respond to the local economic changes we study. As with the Larkin
instrument, the exclusion restriction is assessed in the validation
package (Sections~\ref{sec:pre_balance} and~\ref{sec:pre_trends}).
