...
#
# SEED STREAMS (cr-review PR #117 should-fix 2; PR #124 should-fix 3):
# variant "base" uses set.seed(recentering_seed + s) -- the SAME
# stream as the settlement engine (diagnostic_roadtiming_draws.R);
# within-design RI is unaffected, but draw-by-draw CROSS-design
# statistics against the settlement draws would inherit correlated
# Monte Carlo error (documented, accepted; regenerating would need an
# offset). Variant "growth" already uses its own stream,
# set.seed(recentering_seed + 300000 + s), so growth-vs-base
# comparisons carry independent Monte Carlo error by construction.
#
