...
main <- function() {
    ...
    for (out in outcomes) {
        ...
        f_stats[[y]] <- list(
            # ROBUST, not classical, since PR #162: these specifications
            # use HC1, so a homoskedastic first-stage F sat beside
            # heteroskedasticity-robust standard errors and carried the
            # same row label as Table 8's robust one. Coauthor's call
            # (2026-09-17). The visible consequence is the IV-Hypo
            # column, which falls from ~6.7 to ~4.0: that is the number
            # consistent with the errors reported next to it.
            lp   = fitstat_F_robust(fits[["IV-LP"]]),
            hypo = fitstat_F_robust(fits[["IV-H"]]),
            both = fitstat_F_robust(fits[["IV-B"]]),
            # Overidentification, IV-Both only (the test does not exist
            # with one instrument). Both the classical Sargan and the
    ...
    for (out in outcomes) {
        ...
        for (spec in c("OLS", "IV-LP", "IV-H", "IV-B")) {
            ...
            csv_rows[[length(csv_rows) + 1L]] <- data.frame(
                ...
                n_obs    = nobs(m),
                # Robust, matching the table row (PR #162).
