...
main <- function() {
    ...
    tex_lines <- c(tex_lines[-length(tex_lines)],
        ...
        paste0("\\emph{Notes}: Each cell is one regression of the row outcome ",
               ...
               "noted in italics. Panel~A reproduces the IV-Both estimates of ",
               "Table~\\ref{tab:sectoral_iv}. The two counterfactual measures ",
               "are never in the same regression. Each panel's first-stage $F$ and ",
