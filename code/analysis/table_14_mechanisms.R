...
main <- function() {
    ...
    # ----- LaTeX output -----
    # Displayed columns looked up by spec_id, not row position, so
    # appending CSV-only rows (like (1cs)) can never shift the table.
    tex_cols <- c("(1)", "(2)", "(3)", "(4)")
    dfx <- df[match(tex_cols, df$spec_id), ]
    stopifnot(!anyNA(dfx$spec_id))
    tex_lines <- c(
        ...
        sprintf("$\\Delta \\ln \\mathrm{MA}^{\\mathrm{full}}$ & %s & %s & %s & %s \\\\",
                tex_cell(dfx$ma_est[1], dfx$ma_se[1], dfx$ma_p[1]),
                tex_cell(dfx$ma_est[2], dfx$ma_se[2], dfx$ma_p[2]),
                tex_cell(dfx$ma_est[3], dfx$ma_se[3], dfx$ma_p[3]),
                tex_cell(dfx$ma_est[4], dfx$ma_se[4], dfx$ma_p[4])),
        "\\midrule",
        "\\multicolumn{5}{l}{\\textit{Local infrastructure $Z_i$:}} \\\\",
        sprintf("$\\Delta$ rail km (1960-1986) & %s & %s & %s & %s \\\\",
                tex_cell_or_blank(dfx$chg_rail_est[1], dfx$chg_rail_se[1]),
                tex_cell_or_blank(dfx$chg_rail_est[2], dfx$chg_rail_se[2]),
                tex_cell_or_blank(dfx$chg_rail_est[3], dfx$chg_rail_se[3]),
                tex_cell_or_blank(dfx$chg_rail_est[4], dfx$chg_rail_se[4])),
        sprintf("$\\Delta$ road km (1954-1986) & %s & %s & %s & %s \\\\",
                tex_cell_or_blank(dfx$chg_road_est[1], dfx$chg_road_se[1]),
                tex_cell_or_blank(dfx$chg_road_est[2], dfx$chg_road_se[2]),
                tex_cell_or_blank(dfx$chg_road_est[3], dfx$chg_road_se[3]),
                tex_cell_or_blank(dfx$chg_road_est[4], dfx$chg_road_se[4])),
        sprintf("Lost all rails by 1986 & %s & %s & %s & %s \\\\",
                tex_cell_or_blank(dfx$lost_rail_est[1], dfx$lost_rail_se[1]),
                tex_cell_or_blank(dfx$lost_rail_est[2], dfx$lost_rail_se[2]),
                tex_cell_or_blank(dfx$lost_rail_est[3], dfx$lost_rail_se[3]),
                tex_cell_or_blank(dfx$lost_rail_est[4], dfx$lost_rail_se[4])),
        sprintf("Gained first paved or gravel road & %s & %s & %s & %s \\\\",
                tex_cell_or_blank(dfx$gain_road_est[1], dfx$gain_road_se[1]),
                tex_cell_or_blank(dfx$gain_road_est[2], dfx$gain_road_se[2]),
                tex_cell_or_blank(dfx$gain_road_est[3], dfx$gain_road_se[3]),
                tex_cell_or_blank(dfx$gain_road_est[4], dfx$gain_road_se[4])),
        "\\midrule",
        sprintf("First-stage $F$ & %.1f & %.1f & %.1f & %.1f \\\\",
                dfx$ma_F[1], dfx$ma_F[2], dfx$ma_F[3], dfx$ma_F[4]),
        sprintf("Observations & %d & %d & %d & %d \\\\",
                dfx$n_obs[1], dfx$n_obs[2], dfx$n_obs[3], dfx$n_obs[4]),
        "\\bottomrule",
        "\\end{tabular}",
        "",
        "\\footnotesize",
